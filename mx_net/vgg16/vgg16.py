...
path = 'http://data.mxnet.io/models/imagenet/vgg/'
[mx.test_utils.download(path + 'vgg16-symbol.json'),
 mx.test_utils.download(path + 'vgg16-0000.params'),
...
def predict(img_path, ground_truth):
    ...
    for i in a[0:5]:
        s = ','.join(
            map(str, [labels[i].split(' ')[0], prob[i], img_path, ground_truth, 'mx_net', difference, 'vgg16']))
        print(s)
        csv.write(s)
        csv.write('\n')
