...
class Network:
    ...
    def __init__(self, checkpoint, threads):
        ...
        with self.session.graph.as_default():
            ...
            with tf_slim.arg_scope(tf_slim.nets.vgg.vgg_arg_scope()):
                vgg, _ = tf_slim.nets.vgg.vgg_16(self.images, num_classes=self.CLASSES, is_training=False)

            self.predictions = vgg[0]#tf.argmax(vgg, dimension=1)
...
def predict(image_file, network, ground_truth):
    ...
    for i in list(arr.argsort()[-5:][::-1]):
        ...
        s = ','.join(map(str, [predicted_class, predicted_procent, image_file, ground_truth, framework, difference, 'vgg16']))
        print(s)
        csv.write(s)
        csv.write('\n')
...
if __name__ == "__main__":
    network = Network('../../data/vgg_16.ckpt', 1)
    work('../../result', network)
    csv.close()
